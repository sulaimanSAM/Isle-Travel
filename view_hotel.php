<?php 
if(isset($_GET['id'])){
    $hotel = $conn->query("SELECT * FROM `hotel` where md5(id) = '{$_GET['id']}'");
    if($hotel->num_rows > 0){
        foreach($hotel->fetch_assoc() as $k => $v){
            $$k = $v;
        }
    }
$review = $conn->query("SELECT r.*,concat(firstname,' ',lastname) as name FROM `rate_review` r inner join users u on r.user_id = u.id where r.hotel_id='{$id}' order by unix_timestamp(r.date_created) desc ");
...
if(is_dir(base_app.'uploads/hotel_'.$id)){
    $ofile = scandir(base_app.'uploads/hotel_'.$id);
    foreach($ofile as $img){
        if(in_array($img,array('.','..')))
        continue;
        $files[] = validate_image('uploads/hotel_'.$id.'/'.$img);
    }
}
}
?>
    ...
    $(function(){
        $('#book').click(function(){
            if("<?php echo $_settings->userdata('id') ?>" > 0)
                uni_modal("Book Info","book_hotel_form.php?hotel_id=<?php echo $id ?>");
            else
