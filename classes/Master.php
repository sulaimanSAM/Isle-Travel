...
Class Master extends DBConnection {
	...
	function save_hotel(){
		...
		if(empty($id)){
			$sql = "INSERT INTO `hotel` set {$data} ";
			$save = $this->conn->query($sql);
			$id = $this->conn->insert_id;
		}else{
			$sql = "UPDATE `hotel` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			if(isset($_FILES['img']) && count($_FILES['img']['tmp_name']) > 0){
				...
				$this->conn->query("UPDATE `hotel` set {$data} where id = '{$id}' ");
				foreach($_FILES['img']['tmp_name'] as $k =>$v){
					move_uploaded_file($_FILES['img']['tmp_name'][$k],base_app.'uploads/hotel_'.$id.'/'.$_FILES['img']['name'][$k]);
				}
			}
	...
	function delete_hotel(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `hotel` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			if(is_dir(base_app.'uploads/hotel_'.$id)){
			...
			$this->settings->set_flashdata('success',"Tour hotel successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
